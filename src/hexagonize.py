...
# Base Parameters
TERRAIN_FILE = get_env_var("TERRAIN_FILE", "input/gemeenten/gemeenten.shp")
TERRAIN_LAYER = get_env_var("TERRAIN_LAYER", "points")
FEATURE_GPKG = get_env_var("FEATURE_GPKG", "input/dataset.gpkg")
FEATURE_COLUMN = get_env_var("FEATURE_COLUMN", "_count")
...
# Filter Parameters
FILTER_COLUMN = get_env_var("FILTER_COLUMN", None)
FILTER_CONDITION = get_env_var("FILTER_CONDITION", None)
...
def generate_hex_grid(terrain_boundary):
    ...
    for h3_cell in h3_hexes:
        ...
        if USE_PERLIN:
            ...
            raw_noise = noise([centroid.x / NOISE_VARIATION, centroid.y / NOISE_VARIATION])
...
def aggregate_features(hex_gdf, features):
    ...
    if AGGREGATION_METHOD == "sum":
        hex_gdf["feature_height"] = joined.groupby(joined.index)[FEATURE_COLUMN].sum().fillna(0)
    elif AGGREGATION_METHOD == "mean":
        hex_gdf["feature_height"] = joined.groupby(joined.index)[FEATURE_COLUMN].mean().fillna(0)
    elif AGGREGATION_METHOD == "max":
        hex_gdf["feature_height"] = joined.groupby(joined.index)[FEATURE_COLUMN].max().fillna(0)
    elif AGGREGATION_METHOD == "min":
        hex_gdf["feature_height"] = joined.groupby(joined.index)[FEATURE_COLUMN].min().fillna(0)
...
def hexagonize():
    ...
    # Generate hex grid
    hex_gdf, h3_hexes = generate_hex_grid(terrain_boundary)
