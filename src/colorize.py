...
HEXAGON_BUFFER = get_env_var(
    "HEXAGON_BUFFER", 0.0001, float
)  # Small buffer size in same units as geometry
...
def load_hexagon_data(hexagonized_data=None):
    ...
    if hexagonized_data is None:
        try:
            ...
            logger.error(
                f"Hexagonized data not found at {HEXAGONS_PICKLE}. Please run hexagonize.py first or provide data."
            )
            return None
...
def rasterize_data(
    hex_gdf, buffered_geometries, buffer_only_geometries, width, height, transform
):
    ...
    height_mat = rasterize(
        [
            (geom, value)
            for geom, value in zip(hex_gdf.geometry, hex_gdf.combined_height)
        ],
...
def create_visualization(
    ...
    terrain_mat = rasterize(
        [
            (geom, value)
            for geom, value in zip(hex_gdf.geometry, hex_gdf["terrain_height"])
        ],
        out_shape=(height, width),
        transform=transform,
        fill=np.nan,
    ...
    feature_mat = rasterize(
        [
            (geom, value)
            for geom, value in zip(hex_gdf.geometry, hex_gdf["feature_height"])
        ],
        out_shape=(height, width),
        transform=transform,
        fill=np.nan,
    ...
    ax1.imshow(
        ...
        aspect=geographic_aspect_ratio,
    )
...
def colorize(hexagonized_data=None):
    ...
    minx, miny, maxx, maxy, width, height, geographic_aspect_ratio, transform = (
        calculate_raster_dimensions(hex_gdf)
    )

    # Create buffered geometries
    buffered_geometries, buffered_values, buffer_only_geometries = (
        create_buffered_geometries(hex_gdf)
    )

    # Rasterize data
    height_mat = rasterize_data(
        hex_gdf, buffered_geometries, buffer_only_geometries, width, height, transform
    )
